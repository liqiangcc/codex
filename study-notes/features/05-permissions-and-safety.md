...
2026-07-10 权限对照实验：在相同的最小写入请求下，`read-only + never` 明确拒绝创建 `experiments/20260710-permissions/write-probe.txt`，且文件不存在；`workspace-write + never` 只创建了该目标文件，并写入预期内容 `workspace-write verified`。两次 prompt 都禁止网络、Git 命令和其他文件改动。`workspace-write` 会话还可读取公开系统文件 `/etc/os-release`，因此它不应被误解为“只可读取工作区”；敏感路径需要显式权限策略。该结果证明 sandbox 是实际执行边界，而不只是 prompt 约定；网络对照因不必要的外部资源风险未执行。
